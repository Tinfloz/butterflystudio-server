...
const getAllMasterData = async (_req: Request, res: Response): Promise<void> => {
    try {
        let db = mongoose.connection.db;
        const masterData = await db?.collection("masterdata").find({}).toArray();
        res.status(200).json(resObjectMaker.getOkResponseObject("Master data fetched successfully!", {
            masterData
        }));
    } catch (error: any) {
...
const getMasterDataByName = async (req: Request, res: Response): Promise<void> => {
    try {
        let db = mongoose.connection.db;
        let masterData = await db?.collection("masterdata").find({ name: req.params.name }).toArray();
        res.status(200).json(resObjectMaker.getOkResponseObject("Master data fetched successfully!", masterData));
    } catch (error: any) {
        res.status(error?.status ?? 500).json(error?.jsonBody ?? resObjectMaker.getErrResponseObject("Something went wrong!"));
    }
...
const getMasterDataByType = async (req: Request, res: Response): Promise<void> => {
    try {
        let db = mongoose.connection.db;
        let masterData = await db?.collection("masterdata").find({ type: req.params.type }).toArray();
        res.status(200).json(resObjectMaker.getOkResponseObject("Master data fetched successfully!", masterData));
    } catch (error: any) {
        res.status(error?.status ?? 500).json(error?.jsonBody ?? resObjectMaker.getErrResponseObject("Something went wrong!"));
    }
